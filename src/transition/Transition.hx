...
class Transition {
	...
	public function new(?showTime:Float = 1, ?hideTime:Float = 1, ?showDelay:Float = 0, ?hideDelay:Float = 0, ?startHidden:Bool = true, ?condition:Condition) {
		...
		if (linearEaseNone == null)
			linearEaseNone = new LinearEaseNone();
	...
	function onTweenCountChange() {
		if (globalTweenCount.value == 0)
			globalTransitioning.value = false;
		else
			globalTransitioning.value = true;
	}
	...
	function showTween():Void {
		stopCurrentTween();
		Actuate.tween(this, showTime, {value: 0})
			.onUpdate(privateShowOnUpdate)
			.onComplete(privateShowOnComplete)
			.ease(linearEaseNone);
		privateShowOnStart();
	}
	...
	function hideTween():Void {
		stopCurrentTween();
		Actuate.tween(this, hideTime, {value: 1})
			.onUpdate(privateHideOnUpdate)
			.onComplete(privateHideOnComplete)
			.ease(linearEaseNone);
		privateHideOnStart();
	}

	inline function stopCurrentTween() {
		Actuate.stop(this);
	}
	...
	function set_condition(value:Condition):Condition {
		if (unbindCondition != null)
			unbindCondition();
		unbindCondition = null;
		if (value != null)
			unbindCondition = Bind.toggle(value, show, hide);
